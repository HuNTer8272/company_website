...
function EditProduct() {
  ...
  useEffect(()=>{
    const fetchProducts = async()=>{
     try{
        ...
        const query = productRef.where('id','==',productId);
 ...
 const handlePaste = (e)=>{
    ...
    if(items.length>0){
      for(let i=0;i<items.length;i++){
        ...
        uploadBytes(storageRef,file).then(()=>{
          getDownloadURL(storageRef).then((url)=>{
             setUrl(url);
             console.log("Image successfully using the Upload function")
          }).catch((error)=>{
      ...
      console.log('image succesfully uploaded using the paste function ')
    }
  } catch(error){
...
const handleSubmit = async () => {
  try {
    ...
    const productsRef = firestore.collection('products').doc(productId);
    const productData = {
    ...
    <header style={{ background: `url(${bg}) no-repeat center center / cover` }} className="top">
      <div className="in ml-10">
        <input
          ...
          value={productName === '' ? (products.length > 0 && products[0].name ? products[0].name : '') : productName}
  ...
  {imgSrc !== null ? (
    ...
    <div
      ...
      style={{
        background: `url(${products.length > 0 ? products[0].productImage : ''}) no-repeat center center / cover`,
        border: '1px solid #eee'
      }}
    >
      <input
...
value={productName === '' ? (products.length > 0 && products[0].name ? products[0].name : '') : productName}
            ...
            <div className="description text-lg font-poppins -mt-5 ">
              ...
              <input
                ...
                value={description === '' ? (products.length > 0 && products[0].description ? products[0].description : '') : description}
            ...
            <div className="description flex flex-col text-lg font-poppins mt-10 ">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={catg===''?products.length>0 && products[0].category:catg}
            ...
            <div className="description flex flex-col text-lg font-poppins mt-10 ">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={price===''?products.length>0 && products[0].price:price}
